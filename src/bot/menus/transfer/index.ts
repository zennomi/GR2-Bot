...
import singleTransferMenu from "./singleTransfer";
import WalletModel from "../../../models/wallet";
import config from "../../../config";
import { sendMessage } from "../../../utils/telegram";

const transferMenu = new MenuTemplate<BotContext>(async ctx => {
...
transferMenu.manualRow(createBackMainMenuButtons());

transferMenu.chooseIntoSubmenu('select',
...
transferMenu.interact("Connect Wallet", "connect", {
    do: async ctx => {
...
[1, 5, 10].map((number, index) => {
    transferMenu.interact(`Generate ${number} wallet`, `gen-${number}`, {
...
transferMenu.interact("Remove all", "remove", {
    joinLastRow: true,
    do: async ctx => {
